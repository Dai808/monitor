...
/**
 * @author: DAI
 * @date: Created in 2020/10/21 15:20
 * @description： 测试后返回数据高位在前低位在后。例如实际校验码为0x1561，此类返回0x6115。
 * CRC16校验，把除校验码之外的数据用算法生成校验码和原始校验码对比
 */
public class CRC16Util {
     ...
     * @return boolean
     */
    public static boolean checkCode(byte[] data, String code) {
        int newCode = CRC16Util.calcCrc16(data);
        String newCodeFormat = String.format("0x%04x", newCode);
        String codeFormat = "0x" + code;
        if(codeFormat.equals(newCodeFormat)){
            return true;
        }
        return false;
    }
        ...
        public static void main(String[] args) {
            // 01,03,1a,02,63,00,e9,00,3b,00,06,00,17,00,00,00,3b,00,02,ff,fd,00,1d,00,01,8c,48,02,62,15,61
            // 0x02 05 00 03 FF 00 , crc16=7C 09
            int crc = CRC16Util.calcCrc16(new byte[]{0x01, 0x03, 0x1a, 0x02,  0x63, 0x00, (byte)0xe9, 0x00, 0x3b, 0x00,0x06, 0x00, 0x17, 0x00, 0x00, 0x00,  0x3b, 0x00, 0x02, (byte)0xff,(byte)0xfd, 0x00, 0x1d, 0x00, 0x01, (byte)0x8c, 0x48, 0x02, 0x62
            } );

            int crc1 = CRC16Util.calcCrc16(new byte[]{0x03, 0x03, 0x00, 0x17, 0x00, 0x01});
